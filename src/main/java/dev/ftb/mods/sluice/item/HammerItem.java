...
public class HammerItem extends DiggerItem {
    ...
    public HammerItem(Tiers tier, float attackBase, float attackSpeed, boolean fireResistant) {
        super(attackBase, attackSpeed, tier, new HashSet<>(), !fireResistant
                ? props.addToolType(ToolType.PICKAXE, tier.getLevel()).addToolType(ToolType.SHOVEL, tier.getLevel()).durability(tier.getUses() + (tier.ordinal() + 1) * 100)
                : props.addToolType(ToolType.PICKAXE, tier.getLevel()).addToolType(ToolType.SHOVEL, tier.getLevel()).durability(tier.getUses() + (tier.ordinal() + 1) * 100).fireResistant());
    }
